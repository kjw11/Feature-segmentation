...
if [ "$use_existing_models" == "true" ]; then
  for f in $dev_dir/mean.vec $dev_dir/lda_plda ; do
  ...
  if [ "$length_norm" == "true" ]; then
    run.pl $dev_dir/log/lda_plda.log \
      ...
      "ark:ivector-subtract-global-mean scp:$dev_dir/$vec_type ark:- | transform-vec $dev_dir/lda.mat ark:- ark:- | ivector-normalize-length ark:- ark:- |" \
...
if [ "$length_norm" == "true" ]; then
  run.pl $scores_dir/log/lda_plda_scoring.log \
    ivector-plda-scoring --normalize-length=true \
      ...
      "ark:ivector-mean ark:$enroll_dir/spk2utt scp:$enroll_dir/$vec_type ark:- | ivector-subtract-global-mean $dev_dir/mean.vec ark:- ark:- | transform-vec $dev_dir/lda.mat ark:- ark:- | ivector-normalize-length ark:- ark:- |" \
      "ark:ivector-subtract-global-mean $dev_dir/mean.vec scp:$test_dir/$vec_type ark:- | transform-vec $dev_dir/lda.mat ark:- ark:- | ivector-normalize-length ark:- ark:- |" \
