...
mean_tau=10 # Tau value for updating means "m". (float, default = 10)
variance_tau=10 #  Tau value for updating variances "v". (float, default = 10)
weight_tau=10 # Tau value for updating weight "w". (float, default = 10)
cleanup=true
...
for f in $data/feats.scp $data/spk2utt; do
...
if [ -f $srcdir/final.dubm ]; then # diagonal-covariance in $srcdir
  ...
  ## Set up features.
  $cmd JOB=1:$nj $dir/log/get_spk_vectors.JOB.log \
  concat-segments scp:$data/feats.scp $sdata/JOB/spk2utt ark,scp:$sdata/JOB/spk_vectors.ark,$sdata/JOB/spk_vectors.scp \
   || exit 1;
  ...
  $cmd JOB=1:$nj $dir/log/convert_diag_to_gmm.JOB.log \
    sid/map.sh --binary $binary --update-flags "$update_flags" --mean-tau $mean_tau \
      $sdata/JOB/spk_vectors.scp JOB $srcdir/final.dubm $dir/acc $dir/gmm
  wait
