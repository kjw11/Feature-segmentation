...
for gmmFile in $gmmdir/gmm/*.mod; do
  model=$(basename $gmmFile | sed -e 's:\.: :g' | awk '{print $1}')
  $cmd JOB=1:$nj $scoredir/log/score_gmm.JOB.log \
    gmm-global-get-frame-likes --average=true $gmmdir/gmm/$model.mod "$feats" ark,t:$scoredir/gmm.JOB.score

  for n in $(seq $nj); do
    cat $scoredir/gmm.$n.score
  done > $scoredir/$model.score

  for n in $(seq $nj); do
    rm $scoredir/gmm.$n.score
  done

done

echo GMM-scoring is done!
